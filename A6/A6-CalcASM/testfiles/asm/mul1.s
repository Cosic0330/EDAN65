...
.text
_start:
        pushq %rbp                      //Stackpointer flyttas
        movq %rsp, %rbp
        subq $16, %rsp                  // Allocera för 2st lokala variabler
        movq $3, %rax                   // 3 -> rax             //använder address
        movq %rax, -8(%rbp)             // rax -> local 1
        movq $4, %rax                   // 4 -> rax
        movq %rax, -16(%rbp)            // rax -> local 2

        movq -8(%rbp), %rax             // 3 -> rax
        pushq %rax                      // push rax på stack till nästa block
        movq -16(%rbp), %rax            // 4 -> rax
        movq %rax, %rbx                 // rax -> rbx
        popq %rax                       // 3 -> rax
        imulq %rbx, %rax                // mul
        pushq %rax                      // push mul
        call print                      
        addq $8, %rsp                   //tar port en lokal från stak
        movq %rbp, %rsp                 //return från stackframe
        popq %rbp                       //pop mul
        movq $0, %rdi                   // 0 -> rdi
        movq $60, %rax                  // 60 -> rax
        syscall
